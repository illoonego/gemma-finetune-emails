...
def run_inference(model, tokenizer, email, max_new_tokens=MAX_TOKENS):
    ...
    """
    try:
        # Build a simpler prompt that matches the dataset format
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs,
                max_new_tokens=5,  # Very short for just the intent category
                do_sample=True,
                temperature=0.1,
                top_k=10,
...
def main():
    ...
    for i, email in enumerate(dummy_emails):
        ...
        prediction = run_inference(model, tokenizer, email, max_new_tokens=MAX_TOKENS)
